...
public class GoogleParser extends XMLParser implements Parser {
    ...
    public Route parse() {
        ...
        try {
            ...
            final JSONObject jsonOverviewPolyline = jsonRoute.getJSONObject("overview_polyline");
            route.setPolyline(jsonOverviewPolyline.optString("points"));
            route.addPoints(decodePolyLine(jsonOverviewPolyline.getString("points")));

            int duration = 0;
            for (int i=0; i< jsonRoute.getJSONArray("legs").length();i ++){
                duration += jsonRoute.getJSONArray("legs").getJSONObject(i).getJSONObject("duration").getInt("value");
            }

            double distance =0;
            for (int i=0; i< jsonRoute.getJSONArray("legs").length();i ++){
                distance += jsonRoute.getJSONArray("legs").getJSONObject(i).getJSONObject("distance").getInt("value");
            }

            route.setDurationText(String.valueOf((duration/60)+1));
            route.setDistanceText(String.valueOf((duration/1000)));
